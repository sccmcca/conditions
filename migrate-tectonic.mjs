...
const migratedData = data.map(item => {
  ...
  // Initialize new categories, preserving existing interaction
  newItem.element = [];
  newItem.form = [];
  // Preserve existing interaction values
  newItem.interaction = item.interaction ? [...item.interaction] : [];
...
console.log('Migration complete. Updated static/metadata.json (preserving existing interaction values)');
